...
const archiveRaidProgress=raid=>{const total=Number(raid.totalBosses??0);const row=(label,short,value,className)=>{const done=Number(value??0),percent=total?Math.min(100,done/total*100):0,complete=total>0&&done>=total;return `<div class="archive-progress-row ${className} ${complete?"complete":""}" style="--archive-progress:${percent}%"><div><span><i>${short}</i>${label}</span><b>${esc(done)} / ${esc(total||"–")}</b></div><em><u></u></em>${className==="mythic"&&complete?`<small>VOLLSTÄNDIG MYTHISCH</small>`:""}</div>`};return `<div class="archive-progress">${row("Normal","N",raid.normalKilled,"normal")}${row("Heroisch","H",raid.heroicKilled,"heroic")}${row("Mythisch","M",raid.mythicKilled,"mythic")}</div>`};
