...
function renderCurrentProgress(character){
  ...
  const bossMode=(boss,label,className)=>{const kills=difficultyValue(boss.difficulties,label.toLowerCase());return `<span class="boss-mode ${className} ${kills>0?"done":""}">${esc(label)} ${kills>0?`✓${kills>1?` ×${esc(kills)}`:""}`:"–"}</span>`};
  const weeklyRaids=(weekly?.raidBosses||[]).map(raid=>{const raidData=(character?.raids||[]).find(entry=>raidKey(entry.slug)===raidKey(raid.raid));const total=raidData?.totalBosses;const bosses=(raid.bosses||[]).map((boss,index)=>`<li class="weekly-boss ${Object.values(boss.difficulties||{}).some(value=>Number(value)>0)?"defeated":""}"><span class="boss-emblem" aria-hidden="true">${index+1}</span><div><strong>${esc(boss.name)}</strong><small>${Object.values(boss.difficulties||{}).some(value=>Number(value)>0)?"Diese Woche besiegt":"Diese Woche noch offen"}</small></div><div class="boss-modes">${bossMode(boss,"Normal","normal")}${bossMode(boss,"Heroisch","heroic")}${bossMode(boss,"Mythisch","mythic")}</div></li>`).join("");return `<div class="weekly-raid-row"><div class="weekly-raid-title"><strong>${esc(raid.raid)}</strong><small>Ausschließlich Kills der aktuellen Resetwoche</small></div><div class="difficulty-grid">${difficultyCard("Normal","N",difficultyValue(raid.difficulties,"normal"),total,"normal")}${difficultyCard("Heroisch","H",difficultyValue(raid.difficulties,"heroic"),total,"heroic")}${difficultyCard("Mythisch","M",difficultyValue(raid.difficulties,"mythic"),total,"mythic")}</div>${bosses?`<ul class="weekly-boss-list">${bosses}</ul>`:`<p class="empty">Bossdetails werden mit der nächsten Blizzard-Aktualisierung ergänzt.</p>`}</div>`}).join("")||`<p class="empty">Noch keine Raidboss-Kills in dieser Resetwoche.</p>`;
