...
function renderCurrentProgress(character){
  ...
  const weeklyRaids=(weekly?.raidBosses||[]).map(raid=>{const raidData=(character?.raids||[]).find(entry=>raidKey(entry.slug)===raidKey(raid.raid));const total=raidData?.totalBosses;return `<div class="weekly-raid-row"><strong>${esc(raid.raid)}</strong><div class="difficulty-grid">${difficultyCard("Normal","N",difficultyValue(raid.difficulties,"normal"),total,"normal")}${difficultyCard("Heroisch","H",difficultyValue(raid.difficulties,"heroic"),total,"heroic")}${difficultyCard("Mythisch","M",difficultyValue(raid.difficulties,"mythic"),total,"mythic")}</div></div>`}).join("")||`<p class="empty">Noch keine Raidboss-Kills in dieser Resetwoche.</p>`;
